...
func main() {
	...
	if r != nil {
		...
		for {
			...
			if connerr != nil {
				fmt.Printf("error while opening db con -1 %s\n", connerr)
			} else {
				newId := 0
...
// the match function
func match(buy Order, db *sql.DB) bool {
	var id int
	var orderId string
	var from string
	var amt string
	...
	sqlStatement := `SELECT id, order_id, _from, amt, pmt_method FROM order_info WHERE order_type = $1 AND amt = $2 AND matched = $3 ORDER BY created_at DESC LIMIT 1;`
	row := db.QueryRow(sqlStatement, "sell", buy.Amt, false)

	switch err := row.Scan(&id, &orderId, &from, &amt, &pmt_method); err {
	case sql.ErrNoRows:
	...
	case nil:
		...
		matchOrder.OrderId = buy.OrderId
		matchOrder.OrderType = buy.OrderType
		matchOrder.Amt = buy.Amt
		matchOrder.From = from
		matchOrder.To = buy.To
		matchOrder.PmtMethod = buy.PmtMethod
		matchOrder.SellOrderId = orderId
		matchOrder.PayWith = buy.PmtMethod
		matchOrder.PutProceeds = pmt_method
		matchOrder.Status = "MATCHED"
		matchOrder.CreatedAt = time.Now()
	...
	default:
		fmt.Printf("error while opening db con -2 %s\n", err)
	}
